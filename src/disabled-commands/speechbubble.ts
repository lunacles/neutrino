// TODO: Fix this command... eventually
// a recent discord.js update (seems) to have removed a core component of this command
// as far as i can tell, they removed the ability to get the button label from it's data
// i could be wrong, and if i am please feel free to let me know in this server https://discord.gg/qSFvC2bH2v
...
const SpeechBubble: CommandInterface = {
  ...
  async execute(interaction: ChatInputCommandInteraction<CacheType>): Promise<void> {
    await interaction.followUp({
      content: 'This command is currently disabled due to a recent discord.js update that broke a backbone of this command in the most stupid way feasible.',
    ...
    const collector: Component = interaction.channel.createMessageComponentCollector({
      filter: observer.componentsFilter(buttons.flatMap(action => action.components.flatMap(button => {
        // why the fuck did discord remove the ability to get the button label?????
