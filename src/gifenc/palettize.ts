...
import { RGBTuple, RGBValue, Format, Pair, PaletteValue } from 'types.d.js'
...
const Palettize = {
  ...
  applyPalette(rgba: Uint8ClampedArray, palette: Array<PaletteValue>, format: typeof Format[keyof typeof Format] = Format.RGB565): Uint8Array {
    ...
    if (format === Format.RGBA4444) {
      ...
      let rgb888ToKey = format === Format.RGB444 ? RGBPacker.rgb888ToRGB444 : RGBPacker.rgb888ToRGB565
  ...
  nearestColorIndexWithDistance(colors: Array<PaletteValue>, pixel: Array<number>, distanceFn = Color.euclideanDistSqrd): Pair {
    let minDist: number = Infinity
    let minDistIndex: number = -1
    for (let j: number = 0; j < colors.length; j++) {
      let paletteColor: PaletteValue = colors[j]
