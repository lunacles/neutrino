...
describe('Binary Heap', () => {
  ...
  it('Builds a binary heap from provided data', () => {
    Heap.build(data)
    expect(Heap.heap).toStrictEqual([ 0, 1, 4, 2, 3, 8, 7, 6, 9, 5 ])
  })

  it('Checks if a value belongs in the heap', () => {
    expect(Heap.belongs(11)).toBeFalsy()
    expect(Heap.belongs(-1)).toBeTruthy()
  })

  it('Removes the last element from the heap and returns it', () => {
    Heap.pop()
    expect(Heap.pop()).toBe(9)
  })

  it('Pushes an element to the heap and returns the heap length', () => {
    expect(Heap.push(-1)).toBe(9)
  })

  it('Inserts an element into the heap and rebuilds it', () => {
    Heap.insert(-2)
